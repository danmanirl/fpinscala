...
object MyModule {
  ...
  def fib(n: Int): Int = {
    def next(x1: Int, x2: Int, count: Int): Int = {
      if(count == n) x1 else next(x2, x1+x2, count+1)
    }
    next(0,1,0)
  }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A,A) => Boolean): Boolean = {
    def checkFrom(n: Int): Boolean = {
      if(n >= as.length || as.length <= 1) true else gt(as(n), as(n-1)) && checkFrom(n+1)
    }
    checkFrom(1)
  }
  ...
  // Exercise 4: Implement `uncurry`f(g
